...
import { shiftTypes, statusColorMap, statuses, type CalendarFilters } from '@/helpers/common'
import { ChevronLeft, ChevronRight, EllipsisVertical, Grid3X3 } from 'lucide-react'
import { DropdownMenu } from '../ui/dropdown-menu'
import { DropdownMenuContent, DropdownMenuGroup, DropdownMenuItem, DropdownMenuSeparator, DropdownMenuTrigger } from '@/components/ui/dropdown-menu'
...
export default function CalendarHeader({
  ...
  const BTN_CLASSES = "px-4 py-2 h-[40px] bg-white border text-foreground rounded-[4px] hover:bg-blue-600 hover:text-white cursor-pointer"
  return (
    <div className="flex items-center justify-between mb-4 z-10 relative">
      <div className="flex gap-2">
            ...
            <SelectGroup>
                {statuses.map((s) => (
                    <SelectItem value={s.id}>
                        <div className={`w-2 h-2 rounded-full ${statusColorMap[s.id]}`} />
        ...
        <Select value={filters.shiftType} onValueChange={(val) => setFilters({...filters, shiftType: val})}>
          <SelectTrigger className='cursor-pointer ring-0! h-[40px]! outline-none! shadow-none! border bg-white rounded-[4px]'>
      ...
      <div className="flex items-center gap-3">
        ...
        <DropdownMenu>
            ...
            <DropdownMenuContent sideOffset={4} align='end' className='bg-white py-2 px-0 rounded-sm w-[220px] z-10 shadow-sm border'>
